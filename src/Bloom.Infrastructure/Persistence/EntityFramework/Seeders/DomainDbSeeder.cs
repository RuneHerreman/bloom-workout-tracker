...
public class DomainDbSeeder(DomainDbContext context, ILogger<DomainDbSeeder> logger, IPasswordHasher passwordHasher)
{
    ...
    private async Task SeedExercises()
    {
        ...
        var allExercises = new List<Exercise>
        {
            ...
            // ============================================================
// NEW EXERCISES TO ADD — paste these into the allExercises list
// Continues from ID ...490079 (Machine Row)
// ============================================================

// ——————————————————————————————————————————————
// CHEST — machine / dumbbell / cable / bodyweight variants
// ——————————————————————————————————————————————
Exercise.Create("Machine Row", "A compound pull exercise performed on a plate-loaded or selectorised row machine, providing a fixed movement path that isolates the mid-back while reducing lower-back demand compared to free-weight rows.", ExerciseType.Strength, ["Back"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490080"))),
Exercise.Create("Dumbbell Bench Press", "A compound horizontal press using dumbbells, allowing a greater range of motion and independent arm movement compared to the barbell version, which helps address strength imbalances and increases pec stretch at the bottom.", ExerciseType.Strength, ["Chest"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490081"))),
Exercise.Create("Machine Chest Fly", "A pec-deck or fly machine exercise that keeps the arms in a fixed arc, providing constant tension on the chest through the full range of motion without the stabilisation demands of free weights.", ExerciseType.Strength, ["Chest"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490082"))),
Exercise.Create("Low Cable Fly", "A cable fly performed with the pulleys set low and hands sweeping upward, emphasising the clavicular (upper) fibres of the pectoralis major while maintaining constant cable tension throughout the movement.", ExerciseType.Strength, ["Chest"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490083"))),
Exercise.Create("Push Up", "A bodyweight horizontal push exercise where the hands support the body in a plank position and the body is lowered to the floor, targeting the chest, front delts and triceps with significant core stabilisation demands.", ExerciseType.Strength, ["Chest"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490084"))),
Exercise.Create("Landmine Press", "A pressing movement using a barbell anchored at one end, angled roughly 45 degrees, providing a shoulder-friendly arc that targets the upper chest and front delts while allowing heavy loading with minimal joint stress.", ExerciseType.Strength, ["Chest"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490085"))),
Exercise.Create("Decline Dumbbell Press", "A dumbbell press performed on a decline bench (15-30 degrees), shifting emphasis to the sternal (lower) head of the pectoralis major while allowing independent arm movement and a deeper stretch.", ExerciseType.Strength, ["Chest"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490086"))),
Exercise.Create("Smith Machine Bench Press", "A horizontal press on the Smith machine, which constrains the bar to a vertical path, reducing stabiliser demand and allowing focus on chest activation — useful for training to failure without a spotter.", ExerciseType.Strength, ["Chest"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490087"))),
Exercise.Create("Incline Cable Fly", "A cable fly with an incline bench positioned between two pulleys set low, combining the constant tension of cables with an upward pressing arc that biases the upper pectorals.", ExerciseType.Strength, ["Chest"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490088"))),
Exercise.Create("Dumbbell Pullover", "A single-joint movement lying perpendicular on a bench, lowering a dumbbell behind the head in an arc, stretching and loading the pecs and lats through a long range of motion while expanding the ribcage.", ExerciseType.Strength, ["Chest"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490089"))),
...
// ——————————————————————————————————————————————
Exercise.Create("T-Bar Row", "A rowing movement with one end of a barbell anchored, using a V-grip handle, loading the mid-back and lats heavily while allowing heavier weights than single-arm variations due to bilateral force production.", ExerciseType.Strength, ["Back"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490100"))),
Exercise.Create("Chest Supported Row", "A prone row on an incline bench (30-45 degrees), eliminating lower-back involvement entirely and isolating the mid-back, rear delts and biceps — excellent for building thickness without spinal loading.", ExerciseType.Strength, ["Back"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490101"))),
...
Exercise.Create("Cable Hammer Curl", "A hammer curl using a rope attachment on a low cable, maintaining constant tension on the brachialis and brachioradialis through the full range, unlike dumbbells which lose tension at the top and bottom.", ExerciseType.Strength, ["Biceps"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490119"))),
Exercise.Create("Bayesian Cable Curl", "A single-arm cable curl performed with the pulley behind the body, placing the shoulder in extension and creating a deep stretch on the long head of the biceps at the bottom of each rep.", ExerciseType.Strength, ["Biceps"], EntityId.New<ExerciseId>(Guid.Parse("019d059e-d220-71db-8a1a-ec7569490120"))),
